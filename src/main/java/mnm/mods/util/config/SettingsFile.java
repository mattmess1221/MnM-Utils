...
/**
 * Used for creating settings and saving/loading them in the JSON format. Start by registering settings using
 * {@link #registerSetting(String, SettingValue)}. If your setting requires special handling for serialization, override
 * {@link #setupGson(GsonBuilder)} and use it to customize the {@link Gson} object to your liking.
 *
...
public abstract class SettingsFile<T extends SettingsFile<T>> extends SettingObject<T> {
    ...
    /**
     * Creates settings with a settings file at root/name.json. Settings are not automatically loaded.
     *
    ...
    /**
     * Called before the {@link Gson} object is created in order to allow customization of it.
     *
    ...
    public final void loadSettingsFile() {
        ...
        try {
            file.createNewFile();
            jobj = gson.fromJson(reader = new FileReader(file), JsonObject.class);
        } catch (Exception e) {
